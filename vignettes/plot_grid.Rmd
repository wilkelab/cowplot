...
A few common plot formats are handled, which includes the classes `recordedplot` (returned by `recordPlot()`), `ggplot`, and `gtable`, the latter of which being easy to create from e.g. `grob` and `gTree` objects. 

```{r, message=FALSE, fig.show='hide', collapse=TRUE}
plot(sqrt)
recordedplot <- recordPlot()
```

Since the code to create a `recordedPlot` object would ordinarily create a unwanted plot or file, a function creating a `graphics` based plot (`plot`, `image`, ...) can be passed as well:
...
plot_grid(g.mpg, recordedplot, plotfunc, g_table)
```
