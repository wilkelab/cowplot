...
draw_vector_image <- function(image, x = 0, y = 0, width = 1, height = 1, scale = 1, clip = "inherit", .gridSVG = FALSE) {
  ...
  # otherwise we try to read it in with readPicture()
    ...
    # check whether file type is Cairo SVG (look for pattern in the warning message)
    isCairoFile <- FALSE
    xml <- try(XML::xmlParse(image))
    if (!inherits(xml, "try-error")) {
      msg <- tryCatch(grImport2:::checkValidSVG(xml), warning=function(w) as.character(w) )
      msg <- if (length(msg) == 0) "" else msg
      if (!grepl("This picture was not generated by Cairo graphics", msg))
        isCairoFile <- TRUE
    }
    if (isCairoFile)
      image_data <- grImport2::readPicture(image)
    else {
