...
#' Draw a vector image
#'
#' Places a vector image somewhere onto the drawing canvas. By default, coordinates run from
#' 0 to 1, and the point (0, 0) is in the lower left corner of the canvas. Requires the `grImport2` and `grConvert`
#' package to work, and fails gracefully if that package is not installed.
#' @param image The image to place. Can be a file path, or an image previously created by `grImport2::readPicture()`.
#' @param x The x location of the lower left corner of the image.
#' @param y The y location of the lower left corner of the image.
#' @param width Width of the image.
#' @param height Height of the image.
...
#' library(ggplot2)
#' library(cowplot)
#' cairoFile <- system.file("SVG", "lwd-rsvg.svg", package="grImport2")
#' svgFile <- system.file("SVG", "lwd.svg", package="grImport2")
#' pdfFile <- system.file("tests", "New_Zealand.pdf", package="grConvert")
#'
#' ggdraw() + draw_vector_image(grImport2::readPicture(cairoFile))
#' dev.off()
#' ggdraw() + draw_vector_image(cairoFile)
#' dev.off()
#' ggdraw() + draw_vector_image(svgFile)
#' dev.off()
#' ggdraw() + draw_vector_image(pdfFile)
#' dev.off()
#'
#' plot_grid(NULL,
#'           ggdraw() + draw_vector_image(otherFile),
#'           NULL,
#'           NULL,
#'           ncol=2, nrow=2, labels="AUTO")
#' @export
draw_vector_image <- function(image, x = 0, y = 0, width = 1, height = 1, scale = 1, clip = "inherit", .gridSVG = FALSE) {
  if (!requireNamespace("grImport2", quietly = TRUE)){
    draw_grob(grid::nullGrob(), x, y, width, height)
    stop("Package `grImport2` is required to draw vector images. Image not drawn.", call. = FALSE)
  }
  else { # patch a bug of grImport2 (grobify() raises an error if called before grid.picture())
    grImport2:::setPrefix(grImport2:::generateNewPrefix())
  }
  # if we're given an image, we just use it
  if (methods::is(image, "Picture")) {
    image_data <- image
  }
  # otherwise we try to read it in with readPicture()
  else {
    stopifnot(file.exists(image), length(file)==1)
    image_data <- tryCatch(grImport2::readPicture(image),
                           error=function(e) e,   # catch error of XML::xmlParse() in case file is not XML
                           warning=function(w) w) # catch warning grImport2::readPicture() to check whether Cairo compatible in case file is SVG
    # if reading in failed, process with grConvert::convertPicture()
    if ( inherits(image_data, "error") ||
         (if (inherits(image_data, "warning")) grepl("This picture was not generated by Cairo graphics", image_data) else FALSE) ) {
      # check grConvert dependency
      if (!requireNamespace("grConvert", quietly = TRUE)){
        draw_grob(grid::nullGrob(), x, y, width, height)
        stop("Package `grConvert` is required to convert vector images which are not Cairo SVG. Image not drawn.", call. = FALSE)
      }
      cairo_image_path <- tempfile(fileext = '.svg')
      try(suppressMessages(grConvert::convertPicture(image, cairo_image_path)))
      if (!file.exists(cairo_image_path)) {
        draw_grob(grid::nullGrob(), x, y, width, height)
        stop("Conversion to Cairo SVG failed (try to run `grConvert::convertPicture` to debug). Image not drawn.", call. = FALSE)
      }
      image_data <- grImport2::readPicture(cairo_image_path)
      file.remove(cairo_image_path)
    }
  }
  g <- grImport2::grobify(image_data)
  draw_grob(g, x, y, width, height, scale, clip)
}
