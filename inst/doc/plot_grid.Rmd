...
```{r, message=FALSE, fig.width=3.4, fig.height=5.1}
...
Finally, it is possible to scale the overall size of the plots up or down. This can be useful if the plots have too much or too little margin. Note that a `scale` setting > 1 may not work in combination with the `labels` argument if it would move the labels off the plot.
```{r, message=FALSE, fig.width=6.8, fig.height=2.55}
...
(Notice how we used `rel_heights` to make to bottom row higher than the top row, and `scale` to introduce a little extra white-space around the plots.)
